        ...
        <>
            <div className="w-full h-full">
                <div className="relative w-full h-full">
                    ...
                    <div className={`absolute duration-150 bg-white ${burger ? 'left-[var(--burger--folder)]' : 'left-0'} right-0 top-0 bottom-0`}>
                        <div className="absolute w-full h-[var(--burger--top)] border-b text-supergray font-semibold text-base px-6">
                            <div className="absolute top-1/2 -translate-y-1/2">
                                {title}
                            </div>
                        </div>
                        <div className="relative top-[var(--burger--top)] bottom-0 left-0 right-0">
                            <button onClick={()=>setBurger(!burger)}>btn</button>
                            {children}
                        </div>
                    </div>
                </div>

                {/*<div className={`absolute shadow-xl duration-150 bg-white  right-0`}>
                    <div className="absolute w-full h-[var(--burger--top)] border-b text-supergray font-semibold text-base px-6">
                        <div className="absolute top-1/2 -translate-y-1/2">
                            {title}
                        </div>
                    </div>
                    <div className="relative w-full top-[var(--burger--top)] bottom-0 left-0 right-0">
                    <button onClick={()=>setBurger(!burger)}>btn</button>
                        {children}
                    </div>
                </div>*/}
    ...
    return (
        <>
            <div className="relative w-full h-full">
                ...
                <div className="absolute left-[var(--burger--folder)] right-0 w-full">
                    {children}
                </div>
                <div className={`absolute shadow-xl duration-150 bg-white ${burger ? 'left-[var(--burger--folder)]' : 'left-0'} right-0`}>
                    <div className="absolute w-full h-[var(--burger--top)] border-b text-supergray font-semibold text-base px-6">
                        <div className="absolute top-1/2 -translate-y-1/2">
                            {title}
                        </div>
